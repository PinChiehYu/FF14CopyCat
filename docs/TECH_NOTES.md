...
- 驗證：
  ...
  - 繁中服 7.2 的內容搭配 **7.3 的職業技能調整**（使用者確認），因此 `patchAt()` 回傳內容版本與技能版本（`GamePatch.content`／`jobs`），規則依 `jobs` 選用。第一版把繁中服 7.2 當成技能 7.2，繪靈（FXLkqaK32PhQH8Ac #1）套用了 7.2 的特別規則（不合格原因變為「GCD 只打 8 個」）；改為技能 7.3 後回到一般規則（4/7，重錘不足 3 次）。7.2 以前的繁中服技能版本未確認，視為與內容版本相同。
