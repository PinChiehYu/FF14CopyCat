...
3. 依我的時間排序後取參考時間嚴格遞增的最長子序列（LIS）；同一時間點只留一個錨點。
   - 再去掉孤立錨點（`dropSpikes()`）：時間差（ref − mine）和前 3 個、後 3 個錨點時間差的中位數都差 ≥ 3 秒，而前後兩個中位數彼此差 < 3 秒；每次只去掉最離群的一個再重新判斷，直到沒有（連續配錯好幾個時彼此是鄰居，一次判斷只能去掉中間的：M5S `BF76r8yKh4wGaYkm` #1 vs `Kwx3LyFJjz26pYRm` #16 開場 B 面的放入與播放〔42881、42884 等〕連續 3 個錨點差 −19.6～−19.7 秒，改為反覆去掉後時間差範圍由 −19.7～0.1 變為 −1.5～0.1 秒）。第一個與最後一個錨點只有一側：那一側 3 個錨點彼此差 < 3 秒、而它與其中位數差 ≥ 3 秒時也去掉（M5S `BF76r8yKh4wGaYkm` #1 vs `kCcYLfbxnTJ91Md6` #1 尾聲我 4 拍節奏、參考 8 拍節奏，參考較晚的 Quarter Beats〔9:13〕配到我最後一次〔8:54〕，時間差 +18.9 秒，被判成「我快 20.2 秒」）。去掉後 7 組比較（M5S 三組、M7S、M8S 武士／騎士／黑魔）只剩 M8S 的三個真正推進差距（6.7／8.9／11.3 秒）。隨機順序的機制會讓「同一技能的第 n 次」配錯，LIS 只能擋掉順序矛盾的，擋不掉這種；真正的推進前後時間差不同，不會被去掉。實例（2026-09-27）：熱舞綠光 `BF76r8yKh4wGaYkm` #1 vs `YAzxqkpVfBNmcMwj` #1 開場 A 面／B 面先後相反，Flip to B-Side #1 配成我 46.9 ↔ 參考 27.5（時間差 −19.4 秒，前後錨點 −0.3／−0.1 秒），被判成 0:54「我快 9.7 秒」；M7S 武士基準 42406／42405 兩個錨點（424 秒附近，時間差 −15.2 秒，前後 0～0.4 秒）被判成「我快 8.0 秒」。去掉後兩者都消失，M8S 騎士（慢 8.9 秒）與武士（慢 6.7 秒）的推進差距不變。
