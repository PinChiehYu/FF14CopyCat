...
2. 錨點候選＝兩份日誌中「同一技能 ID 的第 n 次施放」；任一邊施放超過 8 次的技能不當錨點。對齊做兩次：第一次（至少 3 個錨點時）以 `variantGroups()` 找出隨機機制組——對齊後兩邊都「沒有同一技能在 5 秒內」的施放中，相距 1.5 秒內、互為最近的一對不同技能歸成一組（union-find，會連鎖，例如二連／三連／四連指向）；第二次把同一組的技能當成同一個機制編號（組內 5 秒內的施放算同一次），我的第 1 次「放入 A 面」就配參考的第 1 次「放入 B 面」。只有一邊施放的（例如 M8S 輸出夠高時被跳過的第二次空間斬 Extraplanar Pursuit）另一邊沒有對應，不會歸組。實測（2026-09-27）找到的組都是同一機制的變化：M8S Stonefang／Windfang、Eminent／Revolutionary Reign、Wolves' Reign 各版本、Hero's Blow 左右；M7S Smash Here／There、Brutish Swing、Lashing Lariat 左右。錨點數增加（例：M5S `WBNDQCdcrP6A1qkv` 119 → 147、M8S 武士 194 → 214），推進差距不變。
3. 依我的時間排序後取參考時間嚴格遞增的最長子序列（LIS）；同一時間點只留一個錨點。
   - 再去掉配錯的錨點（`dropDetours()`）：隨機順序的機制會讓「同一技能的第 n 次」配到另一段機制，LIS 只能擋掉順序矛盾的，擋不掉這種。依時間差（ref − mine）把連續錨點分段（與目前這段中位數差 ≥ 3 秒就開新段），去掉「跳開又跳回」的段：夾在兩段之間、前後兩段時間差相差 < 3 秒、且在我的時間上 ≤ 30 秒；中間段都處理完後，第一段（時間差不接近 0 時，因開打時兩邊同步）或最後一段少於 3 個錨點、而相鄰段至少 3 個時也去掉；每次去掉一段後重新分段。真正的推進是跳過去不回來，不會被去掉。演進（2026-09-27）：先是逐一去掉與前後 3 個錨點都不同的孤立錨點，但實例一再出現連續配錯：熱舞綠光 `BF76r8yKh4wGaYkm` #1 對 `YAzxqkpVfBNmcMwj` #1（開場 A／B 面相反，1 個錨點差 −19.4 秒，誤判 0:54「我快 9.7 秒」）、對 `Kwx3LyFJjz26pYRm` #16（連續 3 個差 −19.7 秒）、對 `WBNDQCdcrP6A1qkv` #16（B 面整段 5 個差 −20.0 秒，自成一群），以及對 `kCcYLfbxnTJ91Md6` #1 的最後一個錨點（尾聲我 4 拍、參考 8 拍，參考 9:13 的 Quarter Beats 配到我 8:54，+18.9 秒，誤判「我快 20.2 秒」）；M7S 武士基準 424 秒附近 2 個錨點差 −15.2 秒（誤判「我快 8.0 秒」）。改為分段後，8 組比較（M5S 四組、M7S、M8S 武士／騎士／黑魔）只剩 M8S 的三個真正推進差距（慢 6.7／8.9／11.3 秒），M5S 四組時間差範圍都在 3 秒內。
4. 戰鬥開始 (0, 0) 為隱含錨點；錨點間線性內插、最後錨點後斜率 1 外推。`mineToRef` 與反向 `refToMine` 共用錨點（`piecewise()`）。
5. `pushDifferences()`：推進差距。相鄰錨點間時間差（ref − mine）跳 ≥ 3 秒，且前一錨點往前 30 秒、後一錨點往後 30 秒內錨點時間差的**中位數**也差 ≥ 3 秒（同號）才算；相距 15 秒內的跳變合併，合併後重算、未達 3 秒的捨棄（先跳開又跳回）。`deltaMs` > 0 表示我較慢推進。`Comparison.tsx` 只取比較範圍內的，傳給建議（`pushAdvice()`）、對齊說明列與時間軸 Boss 列。
