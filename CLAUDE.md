...
- **遊戲版本**：`jobs/patch.ts` 依戰鬥日期對照繁中服的版本日期判斷版本（只處理繁中服日誌；FFLogs 報告沒有版本資訊），並對應到 xivanalysis 規則所用的**國際服版本**（繁中服 7.2 ＝ 國際服 7.3 的職業技能）；技能窗口規則依國際服版本分（`WindowRule.patches`），兩邊各自評分。**繁中服改版時更新 `TC_PATCHES`**。
