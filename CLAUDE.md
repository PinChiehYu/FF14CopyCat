...
- **遊戲版本**：`jobs/patch.ts` 依戰鬥日期對照繁中服的版本日期判斷版本（只處理繁中服日誌；FFLogs 報告沒有版本資訊），分成內容版本與**職業技能版本**（繁中服 7.2 內容＝7.3 技能）；技能窗口規則依技能版本分（`WindowRule.patches`），兩邊各自評分。**繁中服改版時更新 `TC_PATCHES`**。
